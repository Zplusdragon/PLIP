...
class Model(nn.Module):
    ...
    def encode_text(self,text,mask):
        return self.text_encoder(text, mask)

    def forward(self, image,text,masks):
        global_image_out = self.image_encoder(image)
        global_text_out = self.text_encoder(text,masks)
        return global_image_out,global_text_out
...
def Create_model(args):
    ...
    text_encoder = Text_encoder(encoder_type=args.txt_backbone)
    model = Model(image_encoder, text_encoder)
    return model
